...
​		对于两个均服从在$[0,1]^d$上均匀分布的变量x和y，它们之间的距离$||x-y||^2$满足$E[||x-y||^2]=\sqrt{\frac{d}{6}}$。
...
岭回归的优化目标是
$$
\beta^* = argmin_\beta \frac{1}{n} ||y-X\beta||^2_2 + \lambda ||\beta||_2^2
$$
...
2. min 重建误差
   $$
   \frac{1}{n}\sum_{i=1}^n ||X_i-(v^Tx_i)v||^2
   $$
...
\frac{\part \mathcal L_v}{\part v} = XX^Tv - \lambda v = 0 \\
XX^Tv = \lambda v
$$
...
&\Rightarrow \frac{\part \mathcal L}{\part v} = 2S_B v - 2\lambda S_Wv = 0 \\
...
VAE最终的优化目标是
$$
min_{\theta_1, \theta_2}\quad KL[q_{\theta_2}(z|x^i) || p(z)] - \log p_{\theta_1}(x^i|z^i)
$$

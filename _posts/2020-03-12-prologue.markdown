...
至于`\_config.yml`中其他一些基本信息的配置就不赘述了（唠叨一句防止本金鱼脑下次又想不起来，关于Featured Tags，`\_config.yml`有控制多少篇能成为Featured Tags的参数）。

---

3/12 待更新：小标题左侧出现#，理想结果应该是鼠标悬停左边出现一个🔗符号。对比了原作者的最新博客源码，已经改了`\_layouts\post.html`中`anchor.min.js`的源，并且把`always`改成了`hover`，悬停问题解决了，但还是显示井号

3/12 更新：后来刷新的时候就好了，推测是GitHub刷新速度比较慢
...
配置好博客也是挺不容易的，另外也不能辜负Huxpro先生设计这个模板的好意，希望自己以后能勤更新:)
